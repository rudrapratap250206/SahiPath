...
interface TestsViewProps {
  ...
  onTopicHandled?: () => void;
  onTestRecorded?: (topic: string, score: number) => void;
}

export function TestsView({ pendingTopic, onTopicHandled, onTestRecorded }: TestsViewProps) {
  ...
  const handleSubmitScore = async () => {
    ...
    try {
      await recordMutation.mutateAsync({
        data: { name: activeTest.topic, score, date: new Date().toISOString() }
      });

      if (activeTest.notifId) dismissNotification(activeTest.notifId);
      if (onTopicHandled) onTopicHandled();

      // Notify parent to switch to Performance and show review
      if (onTestRecorded) onTestRecorded(activeTest.topic, score);

      setSubmitSuccess(`✅ Score saved: ${score}/100 for "${activeTest.topic}"`);
      setActiveTest(null);
      setScoreInput('');
      refetch();
    } catch (err: any) {
    ...
    <div style={{ padding: '1.5rem', overflowY: 'auto', height: '100%' }}>
      {submitSuccess && (
        <div style={{ marginBottom: '1rem', padding: '0.8rem 1rem', background: 'rgba(0,212,212,0.1)', border: '1px solid var(--sp-accent-teal)', borderRadius: 8, color: 'var(--sp-accent-teal)', fontSize: '0.9rem' }}>
          {submitSuccess}
          <div style={{ fontSize: '0.82rem', marginTop: '0.3rem', color: 'var(--sp-text-secondary)' }}>Switching to Performance to see your review...</div>
        </div>
      )}
        ...
        <div style={{ marginBottom: '1.5rem', padding: '1.2rem', background: 'var(--sp-bg-tertiary)', borderRadius: 10, border: '1px solid var(--sp-accent-teal)' }}>
          ...
          <div style={{ display: 'flex', gap: '0.8rem', alignItems: 'flex-end', flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: '120px' }}>
              ...
              <input
                type="number" placeholder="e.g. 85" value={scoreInput}
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button className="sp-btn-primary" disabled={recordMutation.isPending} onClick={handleSubmitScore} style={{ padding: '0.65rem 1.2rem' }}>
                {recordMutation.isPending ? '⏳ Saving...' : '✅ Submit & View Performance'}
              </button>
              <button className="sp-btn-secondary" onClick={() => { setActiveTest(null); setSubmitError(null); }} style={{ padding: '0.65rem 1rem' }}>Cancel</button>
            </div>
          </div>
      ...
      {upcoming.length > 0 && (
        <div style={{ marginBottom: '1.5rem' }}>
          <h3 style={{ color: 'var(--sp-accent-orange)', marginBottom: '0.8rem' }}>🔔 Pending Tests ({upcoming.length})</h3>
          {upcoming.map(n => (
            <div key={n.id} style={{ background: 'rgba(255, 140, 66, 0.08)', border: '1px solid var(--sp-accent-orange)', borderRadius: 8, padding: '0.8rem 1rem', marginBottom: '0.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontWeight: 600 }}>{n.topic}</div>
                <div style={{ fontSize: '0.8rem', color: 'var(--sp-text-secondary)', marginTop: '0.2rem' }}>Added {new Date(n.scheduledAt).toLocaleString()}</div>
              </div>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <button className="sp-btn-primary" style={{ padding: '0.4rem 0.8rem', fontSize: '0.8rem' }} onClick={() => handleTakeTest(n.topic, n.id)}>📝 Take Test</button>
                <button className="sp-btn-secondary" style={{ padding: '0.4rem 0.8rem', fontSize: '0.8rem' }} onClick={() => dismissNotification(n.id)}>Dismiss</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {tests.length === 0 ? (
        <div style={{ color: 'var(--sp-text-secondary)', textAlign: 'center', padding: '2rem', background: 'var(--sp-bg-tertiary)', borderRadius: 8 }}>
          No tests recorded yet. Your mentor will suggest topics — click "📝 Take Test" to record your score!
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          {tests.map((t: any) => (
            <div key={t.id} style={{ background: 'var(--sp-bg-tertiary)', borderRadius: 8, padding: '0.8rem 1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderLeft: `3px solid ${t.score >= 70 ? 'var(--sp-accent-teal)' : t.score >= 50 ? 'var(--sp-accent-orange)' : 'var(--sp-accent-coral)'}` }}>
              <div>
                <div style={{ fontWeight: 600 }}>{t.name}</div>
                <div style={{ fontSize: '0.8rem', color: 'var(--sp-text-secondary)' }}>{t.date ? new Date(t.date).toLocaleDateString() : 'No date'}</div>
              </div>
