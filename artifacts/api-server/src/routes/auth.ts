...
router.post("/auth/register", async (req, res) => {
  ...
  try {
    ...
    return res.json({ user: { id: user.id, email: user.email, profile: profile ?? null }, token });
  } catch (err: unknown) {
...
router.post("/auth/login", async (req, res) => {
  ...
  try {
    ...
    return res.json({ user: { id: user.id, email: user.email, profile }, token });
  } catch (err: unknown) {
...
router.get("/auth/me", async (req, res) => {
  const rawToken = parseRequestToken(req.headers as { cookie?: string; authorization?: string });
  ...
  try {
    ...
    const profile = user.profile ? (JSON.parse(user.profile) as unknown) : null;
    return res.json({ user: { id: user.id, email: user.email, profile }, token: rawToken });
  } catch (err: unknown) {
